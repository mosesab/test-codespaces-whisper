...
def send_transcript_to_S3(transcript_file_path, retry_attempts=0):
    ...
    try:
        if not os.path.exists(transcript_file_path):
            print(f"Warning: send_transcript_to_S3: Path: {transcript_file_path} does not exist")
            return None
        ...
        print(f"File {transcript_file_path} uploaded successfully to {bucket_name}/{object_name}")
        return response
        ...
        if retry_attempts >= 5:
            ...
            return send_transcript_to_S3(retry_attempts)
